...
# Check needed file
if [ -f "`find ${OriDir}/4_DTIFIT -maxdepth 1 -name "*preproc.nii.gz"`" ]; then
	handleDWI=${OriDir}/4_DTIFIT/*preproc.nii.gz
	handle=$(basename -- $(find ${OriDir}/4_DTIFIT -maxdepth 1 -name "*preproc.nii.gz") | cut -f1 -d '.')
...
5ttgen fsl -nocrop -sgm_amyg_hipp ${OriDir}/5_CSDpreproc/S1_T1proc/T12dwispace.nii.gz ${OriDir}/5_CSDpreproc/S1_T1proc/5tt2dwispace_sgm_amyg_hipp.nii.gz -quiet
...
cp ${OriDir}/4_DTIFIT/${handle}.bvec ${OriDir}/5_CSDpreproc/
cp ${OriDir}/4_DTIFIT/${handle}.bval ${OriDir}/5_CSDpreproc/

mrconvert ${handleDWI} ${OriDir}/5_CSDpreproc/${handle}.mif -fslgrad ${OriDir}/5_CSDpreproc/${handle}.bvec ${OriDir}/5_CSDpreproc/${handle}.bval 
...
bv_num=($(grep '[^[:blank:]]+' -Eo ${OriDir}/5_CSDpreproc/${handle}.bval | sort | uniq -c))
lowb_tmp=0
null_tmp=0
shell_num=0
...
if [[ ${shell_num} -eq 1 ]]; then
	...
	dwi2response tournier ${OriDir}/5_CSDpreproc/${handle}-unbiased.mif ${OriDir}/5_CSDpreproc/S2_Response/response_wm.txt 

	dwi2fod csd ${OriDir}/5_CSDpreproc/${handle}-unbiased.mif ${OriDir}/5_CSDpreproc/S2_Response/response_wm.txt ${OriDir}/5_CSDpreproc/S2_Response/odf_wm.mif -mask ${OriDir}/5_CSDpreproc/dwi_mask.mif
elif [[ ${shell_num} -ge 2 ]]; then
	# for multi-shell data
	dwi2response dhollander ${OriDir}/5_CSDpreproc/${handle}-unbiased.mif ${OriDir}/5_CSDpreproc/S2_Response/response_wm.txt ${OriDir}/5_CSDpreproc/S2_Response/response_gm.txt ${OriDir}/5_CSDpreproc/S2_Response/response_csf.txt

	dwi2fod msmt_csd ${OriDir}/5_CSDpreproc/${handle}-unbiased.mif ${OriDir}/5_CSDpreproc/S2_Response/response_wm.txt ${OriDir}/5_CSDpreproc/S2_Response/odf_wm.mif ${OriDir}/5_CSDpreproc/S2_Response/response_gm.txt ${OriDir}/5_CSDpreproc/S2_Response/odf_gm.mif ${OriDir}/5_CSDpreproc/S2_Response/response_csf.txt ${OriDir}/5_CSDpreproc/S2_Response/odf_csf.mif -mask ${OriDir}/5_CSDpreproc/dwi_mask.mif
else
	echo " Error: Input is not valid..."
fi
...
tckgen ${OriDir}/5_CSDpreproc/S2_Response/odf_wm.mif ${OriDir}/5_CSDpreproc/S3_Tractography/track_DynamicSeed_1M.tck -act ${OriDir}/5_CSDpreproc/S1_T1proc/5tt2dwispace.nii.gz -backtrack -crop_at_gmwmi -seed_dynamic ${OriDir}/5_CSDpreproc/S2_Response/odf_wm.mif -maxlength 250 -minlength 5 -mask ${OriDir}/5_CSDpreproc/dwi_mask.mif -select 1M
