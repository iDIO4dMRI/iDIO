...
for i in HCPMMP DKavg35 AAL3 Yeo400; do 
	applywarp --ref=${OriDir}/5_CSDpreproc/S1_T1proc/T1_BET/${T1name}_bet_Corrected_restore.nii.gz --in=${AtlasDir}/Atlas/${i}_resample_ICBM.nii.gz --warp=${OriDir}/5_CSDpreproc/S1_T1proc/Reg_matrix/mni2str_nonlinear_transf.nii.gz --rel --out=${OriDir}/6_NetworkProc/Atlas/${subjid}_${i}_inT1.nii.gz --interp=nn
...
#Relabel DK atlas
labelconvert ${OriDir}/6_NetworkProc/Atlas/${subjid}_DK_inDWI.nii.gz ${AtlasDir}/colorlabel/FreeSurferColorLUT_DK.txt ${AtlasDir}/colorlabel/fs_default_DK.txt ${OriDir}/6_NetworkProc/Atlas/${subjid}_DK_inDWI.nii.gz -force
