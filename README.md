...
- **trkNum**: Specifies the desired number of streamlines to be selected when generating the tractogram [default = 10M]
  ...
  In order to do the denoise process, the acquired image need to be assured that it did not undergo an image interpolation to prevent change in noise distribution. Remember to turn off the interpolation option on the vendor machine to prevent this issue. Second, Gibbs ring removal relies on full k-space coverage data, data that did not meet the requirement have to be aware of a suboptimal result. Third, evenly distributed interleaved b=0 images may provide a better estimation of the image signal drifting profile. Fourth, we highly recommend acquiring diffusion data using an entire sphere diffusion encoding manner with phase encode reversed image (at least with b=0 images), which may be able to run all corrections of susceptibility-induced distortion, eddy current-induced distortion and subject movement. Some tips in image acquisition with simultaneous multi-slice images, even slices are needed to correct eddy current-induced distortion. Using an odd number of shots per volume may help to prevent the artifactually bright slices in the image (Andersson et al., 2017; Center for Magnetic Resonance Research, 2012). Fifth, if diffusion tensor fitting is required, at least 13 diffusion directions are needed with a b-value lower than 1500 s/mm2. For CSD, multi-shell diffusion weighted imaging is recommended. At least one low b-value shell and one high b-value shell (greater than 2000 s/mm2) with more than 45 diffusion directions is recommended to provide better angular resolution of CSD modelling and have advantages in modelling multi-tissue response function (Tournier et al., 2013). 
